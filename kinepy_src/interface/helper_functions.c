...
uint8_t is_existing_solid(KpConfiguration const * const config, uint32_t const solid_index) {
    return solid_index < config->solid_count;
}


uint8_t is_existing_joint(KpConfiguration const * const config, uint32_t const joint_index) {
...
uint8_t is_valid_relation(KpConfiguration const * const config, uint8_t type, uint32_t joint1, uint32_t joint2) {
    switch (type) {
