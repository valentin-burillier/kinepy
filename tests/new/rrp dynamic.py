...
p2 = sys.add_prismatic(s3, s2, alpha1=np.pi/2, alpha2=np.pi/2)
...
class Load(kp.Interaction):
    def register_actions(self):
        self.add_action(s1, s1.get_point(pa), [[1], [-2]], 0)
...
deplacement = np.linspace(0, 4.9, n)
...
P = np.c_[s1.get_point(pa)[:, frame], s1.get_point(pa)[:, frame] + np.array([1, -2])]
plt.plot(*P)
