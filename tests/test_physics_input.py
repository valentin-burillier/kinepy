...
@physics_input_function_variable(VARIABLE_UNIT, ANGLE)
def test4(u, angle):
    return u, angle


@physics_input_function_variable(VARIABLE_UNIT, VARIABLE_DERIVATIVE)
def test5(u, du):
    return u, du
...
assert test3(1., 180, brandon='Brandou', gravity=(1., 0.)) == (.001, np.math.pi, 'Brandou', (1., 1.), (1., 0))

assert test4(1., 90, phy=LENGTH) == (.001, np.math.pi * .5)
assert test4(90, 90, phy=ANGLE) == (np.math.pi * .5, np.math.pi * .5)
assert test4(9, 90, phy=FORCE) == (90., np.math.pi * .5)
assert test5(90, 90, phy=ANGLE) == (np.math.pi * .5, 90)
