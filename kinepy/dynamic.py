...
def p_p_p(system, cycle, rev1, rev2, rev3, _, eq2, eq3):
    ...
    (s21, p21), (s22, _) = rev2
    (s32, _), (s33, p33) = rev3
    p1, p2, p3 = p1_.point, p2_.point, p3_.point
    ...
    p1_.force = f_13 if p11 else -f_13
    ...
    p2_.force = f_12 if p21 else -f_12
    ...
    p3_.force = f_32 if p33 else -f_32
...
def p_p_g(system, cycle, rev1, rev2, pri, _, eq2, eq3):
    ...
    g.normal, g.torque = (normal, m_23) if s1_ else (-normal, -m_23)
...
def g_p_p(system, cycle, pri, rev1, rev2, _, eq2, eq3):
    ...
    g.normal, g.torque = (normal, m_13) if s1_ else (-normal, -m_13)
    ...
    p1_.force = f_12 if p11 else -f_12
    ...
    p2_.force = f_23 if p22 else -f_23
...
def g_g_p(system, cycle, pri1, pri2, rev, _, eq2, eq3):
    ...
    system.sols[s1p].mech_actions.append(MechanicalAction(n_13, p, m_13))
    system.sols[s1].mech_actions.append(MechanicalAction(-n_13, p, -m_13))
    g1.normal, g1.torque = (normal_13, m_13) if s1_ else (-normal_13, -m_13)
    ...
    g2.normal, g2.torque = (normal_12, m_12) if s2_ else (-normal_12, -m_12)
    ...
    p_.force = f_23 if p32 else -f_23
...
def p_g_g(system, cycle, rev, pri1, pri2, _, eq2, eq3):
    ...
    system.sols[s13].mech_actions.append(MechanicalAction(f_13, p1, 0.))
    system.sols[s11].mech_actions.append(MechanicalAction(-f_13, p1, 0.))
    p_.force = f_13 if p11 else -f_13
    ...
    g1.normal, g1.torque = (normal_12, m_12) if s1_ else (-normal_12, -m_12)
    ...
    g2.normal, g2.torque = (-normal_32, m_23) if s2_ else (normal_32, -m_23)
...
def sp_p(system, cycle, pin, rev, _, eq2):
    ...
    sp.normal = normal_12 if s1_ else -normal_12

    system.sols[s21].mech_actions.append(MechanicalAction(-f_12, p2, 0.))
    system.sols[s22].mech_actions.append(MechanicalAction(f_12, p2, 0.))
    p.force = f_12 if p21 else -f_12
...
def sp_g(system, cycle, pin, pri, _, eq2):
    ...
    sp.normal = normal_12sp if s1_ else -normal_12sp
    ...
    g.normal, g.torque = (normal_12g, m_12) if s2_ else (-normal_12g, -m_12)
...
def t_p(system, cycle, rec, rev, _, eq2):
    ...
    t.torque = m_12 if s1_ else -m_12

    system.sols[s11].mech_actions.append(MechanicalAction(-f_12, p1, 0))
    system.sols[s12].mech_actions.append(MechanicalAction(f_12, p1, 0))
    p.force = f_12 if p11 else -f_12
