...
class GUI(Camera):
    ...
    def draw_pri(self, pri, data):
        ...
        rec = self.real_to_screen(p3[:, frame]) + np.einsum('ik,lk->li', rot(pri.s2.angle[frame] + pri.a2), PRISMATIC * self.scale / self.scale0 * self.zoom)
        pg.draw.polygon(
            self.surface,
            self.background,
            rec,
    ...
    def draw_pin(self, pin, data):
        ...
        rec = self.real_to_screen(p3[:, frame]) + np.einsum('ik,lk->li', rot(pin.s2.angle[frame]), PIN_SLOT * self.scale / self.scale0 * self.zoom)
        pg.draw.polygon(
            self.surface,
            self.background,
            rec,
    ...
    def draw(self):
        ...
        for index, sol in enumerate(self.system.sols):
            ...
            u = unit(-sol.angle[animation_index]) * 40 * self.scale / self.scale0 * self.zoom
        ...
        for j, data in zip(self.system.joints, self.prepared_joints):
            if j.id_ not in self.draw_joint:
                continue
            getattr(self, self.draw_joint[j.id_])(j, data)
